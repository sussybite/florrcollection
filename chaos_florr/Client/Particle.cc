...
static constexpr uint32_t RAINBOW_COLORS[] = {
    0xffff0000, // red
    0xffff8000, // orange
    0xffffff00, // yellow
    0xff00ff00, // green
...
void Particle::add_death_explosion(float x, float y, float radius) {
    int count = (int)(radius * 2) + 10;
    if (count > 40) count = 40;
    for (int i = 0; i < count; ++i) {
        GameParticleEntity part;
        part.x = x + (frand() - 0.5f) * radius;
        part.y = y + (frand() - 0.5f) * radius;
        float speed = 40 + frand() * 80;
        Vector vel = Vector::rand(speed);
        part.x_velocity = vel.x;
        part.y_velocity = vel.y;
        part.radius = 2 + frand() * 4;
        part.opacity = 1;
        part.color = RAINBOW_COLORS[i % 7];
        game_particles.push_back(std::move(part));
    }
}