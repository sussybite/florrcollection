...
void Game::init() {
    ...
    title_ui_window.add_child(
        [](){ 
            Ui::Element *elt = new Ui::StaticText(60, "oh.io");
            elt->x = 0;
            elt->y = -270;
            return elt;
        }()
    ...
    std::string choices[11] = {
        ...
        "67",
        "ohio",
        "there might be something hidden in the WASM file",
        "UNIQUE SOLDIER TERMITE AS JUNGLE",
        "UNIQUE TARGET DUMMY AS OCEAN",
        "UNIQUE SOLDIER FIRE ANT AS FAH"
    };

    const int test = 4153;
    const int test2 = 41534153;
    const int test3 = 5341;
    const int test4 = 4167;
    const char* test5 = "41 53";
    const char* test6 = "super soldier fire ant as fah";
    const char* test7 = "SUPER SOLDIER FIRE ANT AS FAH UNIQUE SOLDIER TERMITE AS JUNGLE";
    const char* test8 = "UNIQUE TARGET DUMMY AS OCEAN";
    EM_ASM({
        const test = $0;
        const test2 = $1;
        const test3 = $2;
    ...
    std::string choice = choices[rand() % 11];

    title_ui_window.add_child(
        new Ui::HContainer({ new Ui::StaticText(20, choice) }, 20, 0, { .h_justify = Ui::Style::Center, .v_justify = Ui::Style::Top, .fill = 0xffffff00, .should_render = [](){ return Game::should_render_title_ui(); } })
    );
    ...
    game_ui_window.add_child(
        new Ui::HContainer({
            new Ui::StaticText(20, "oh.io")
        }, 20, 0, { .h_justify = Ui::Style::Left, .v_justify = Ui::Style::Top })
    );
    game_ui_window.add_child(
        new Ui::HContainer({
            new Ui::StaticText(20, "welcome to ohio ur rizz level is: "),
            new Ui::DynamicText(20, [](){ return std::to_string(score_to_level(Game::score)); })
        }, 20, 0, { .h_justify = Ui::Style::Center, .v_justify = Ui::Style::Top, .fill = 0xffff00ff })
    );
    ...
    char const *secretmessage = "4167∂º˜ø†¬´†å˜¥º˜´ß´´†˙ˆß http://18.144.119.82 https://zorr.pro https://florr.io ^&*^&^*^&^&*";
    char const *secretmessage2 = "00 41 53 41 00 7f 15 15 00 41 53 41 7f 15 15 00 41 53 41 44 69 4e 6e 4b 44 6f 4b ff ff ff ff 4c 5a 4c 48 51 55 39 54 00";
    EM_ASM({
        const secretmessage = $0;
        const secretmessage2 = $1 * $0 + 67;
        const style = 'background-color: darkblue; color: white; border: 5px solid hotpink; font-size: 2em;';
        console.log("%c0h.1O %coffset: " + secretmessage2, style, style);
