...
void DeadFlowerIcon::on_render(Renderer &ctx) {
    ...
    }
    {
        ...
        draw_static_flower(ctx, { .radius = 25, .mouth = 5, .face_flags = (1<<FaceFlags::kDeadEyes), .equip_flags = equip_flags, .color = 3});
    }
    for (uint32_t i = 0; i < physical_loadout.size(); ++i) {
        float angle = 2 * M_PI * i / physical_loadout.size() + 0.5;
        float offset_x = cosf(angle) * 40;
    ...
    ctx.set_glitch_intensity(51.0f);
    ctx.apply_glitch_filter();
}
...
Element *Ui::make_death_main_screen() {
    Ui::Element *continue_button = new Ui::Button(
        145,
        40,
        new Ui::StaticText(28, "ç˜†ˆ˜¨´"),
        [](Element *elt, uint8_t e){ if (e == Ui::kClick && Game::on_game_screen) Game::spawn_in(); },
        [](){ return !Game::in_game(); },
        {.fill = 0xffff0000, .line_width = 5, .round_radius = 3 }
    );
    Ui::Element *container = new Ui::VContainer({
        new Ui::StaticText(25, "You were killed by", { .fill = 0xffff0000 }),
