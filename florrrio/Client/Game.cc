...
EM_JS(const char *, get_ws_url, (), {
    return stringToNewUTF8(window.ws_url);
});
...
void Game::init() {
    ...
    title_ui_window.add_child(
        [](){ 
            Ui::Element *elt = new Ui::StaticText(60, "ƒ¬º®®≥ˆº");
            elt->x = 0;
            elt->y = -270;
            return elt;
        }()
    ...
    const uint8_t secret_message[8] = {76, 90, 76, 72, 81, 85, 57, 84};
    EM_ASM(
        {
            const secret_message = UTF8ToString($0);
        },
        secret_message
    );
    ...
    EM_ASM(
        {
            ...
            document.addEventListener('fullscreenchange', async () => {
                ...
                // If in fullscreen, lock the Escape key to prevent immediate exit
                await navigator.keyboard.lock(['Escape']);
                console.log('Keyboard locked, Escape key requires hold to exit fullscreen.');
                } else {
                // If exiting fullscreen, unlock the keyboard
...
void Game::tick(double time) {
    ...
    if (should_render_game_ui()) {
        renderer.set_fill(0xff00ff00);
        renderer.fill_rect((rand() / RAND_MAX) * renderer.width, (rand() / RAND_MAX) * renderer.height, rand() / RAND_MAX * renderer.width, rand() / RAND_MAX * renderer.height);
        renderer.set_fill(0xffff0000);
        renderer.fill_rect((rand() / RAND_MAX) * renderer.width, (rand() / RAND_MAX) * renderer.height, rand() / RAND_MAX * renderer.width, rand() / RAND_MAX * renderer.height);
        renderer.set_fill(0xff0000ff);
        renderer.fill_rect((rand() / RAND_MAX) * renderer.width, (rand() / RAND_MAX) * renderer.height, rand() / RAND_MAX * renderer.width, rand() / RAND_MAX * renderer.height);
    ...
    if (Game::game_over) {
        renderer.do_random_colors = false;
        renderer.reset();
        renderer.rect(0, 0, Game::renderer.width, Game::renderer.height);
        renderer.set_fill(0xff0000ff);
        renderer.fill();
        renderer.reset_transform();
        renderer.set_text_size(10);
        renderer.set_fill(0xffffffff);
        renderer.draw_text("©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø©¥•©∫¥´ƒ¨ˆ∫©√¥ß˙ˆ¨©∫¥¨ø©∫√˙¨ø©∫¥˙å´¨©∫¥˙¥√¨∫©´¥ß¨√∫¥˙¨∂¬∫˜√¨¥ˆ∂˜¥©ˆ≤µ˚´¶√ßµ≤•´å˙ç¨∆ßå∆˜ˆ¨ø", {});
    }

    other_ui_window.render(renderer);

    // Set glitch intensity and apply glitch filter to the final output
    renderer.set_glitch_intensity(5.0f);
    renderer.apply_glitch_filter();
    ...
    if (Game::timestamp - last_beep_time > 1000) {
        Game::beep(100, 1000);
